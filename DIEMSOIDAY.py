...
def trace_and_count_wires(db, loto_list):
    ...
    # Bảng đếm số lượng dây hội tụ về mỗi số (00-99)
    wire_counts = {f"{i:02d}": 0 for i in range(100)}
    
    # 1. Cập nhật trạng thái thắng/thua cho từng dây dựa trên KQ kỳ này
    for wire_id in range(TOTAL_WIRES):
        w_str = str(wire_id)
        wire = new_matrix[w_str]
        # Con số mà dây này tạo ra (vỏ số)
    ...
    # 2. Thuật toán TRUY VẾT: Đếm số dây đang "SỐNG" (streak_win > 0) hội tụ về từng số
    # Ở đây tao tính toán: Kỳ tới con số nào có nhiều dây vừa nổ ở kỳ này nhất?
    for wire_id in range(TOTAL_WIRES):
        w_str = str(wire_id)
        wire = new_matrix[w_str]
        num_formed = f"{wire_id % 100:02d}"
        
        # Nếu dây này đang trong mạch thắng (vừa ăn kỳ này)
        if wire["streak_win"] > 0:
            # Cộng 1 đơn vị "Dây nối" cho con số đó
...
st.title("⚡ MATRIX 11.449 - THUẬT TOÁN MẬT ĐỘ DÂY")
...
with st.sidebar:
    ...
    if st.button("🔥 CHẠY TRUY VẾT DÂY"):
        ...
        # Chạy thuật toán đếm dây
        new_matrix, counts = trace_and_count_wires(st.session_state['db'], v_loto)
        
        # Trước khi lưu: Tìm hạng của GĐB dựa trên bảng đếm cũ (nếu có)
        rank_val = "-"
        ...
        # Lưu lịch sử
        df_temp = pd.DataFrame(list(counts.items()), columns=['Số', 'Dây']).sort_values(by='Dây', ascending=False)
        def get_hits(target_nums, result_list):
            hits = [n for n in target_nums if n in result_list]
            return f"{sum([result_list.count(n) for n in hits])} ({','.join(sorted(list(set(hits))))})"

        res = {
            "STT": len(st.session_state['history']) + 1,
            "GĐB": st.session_state['gdb_val'],
            "Hạng GĐB": rank_val,
            "Top 10": get_hits(df_temp.head(10)['Số'].tolist(), v_loto),
            "10 Nhì": get_hits(df_temp.iloc[10:20]['Số'].tolist(), v_loto),
            "Vùng Né": get_hits(df_temp.tail(20)['Số'].tolist(), v_loto)
        }
...
if st.session_state['final_counts']:
    ...
    with c1:
        st.subheader("📊 XẾP HẠNG THEO SỐ DÂY")
        ...
        # SẮP XẾP THEO SỐ DÂY CAO NHẤT LÊN ĐẦU
    ...
    with c2:
        ...
        st.divider()
        st.subheader("🎯 DÀN QUÂN MẬT ĐỘ CAO")
        num_pick = st.number_input("Số lượng lấy:", 1, 100, 10)
        st.code(", ".join(df_display.head(num_pick)['Số'].tolist()))
        
        save_data = {"matrix": st.session_state['db'], "history": st.session_state['history'], "last_counts": st.session_state['final_counts']}
        st.download_button("💾 XUẤT JSON", data=json.dumps(save_data), file_name="matrix_trace.json")
