...
st.title("⚡ MATRIX 11.449 - PHÂN TÍCH SIÊU ĐA TẦNG")
...
with st.sidebar:
    ...
    if st.button("XÁC NHẬN & CHẠY"):
        ...
        # Logic đếm NHÁY và liệt kê SỐ NỔ
...
if st.session_state['final_scores']:
    ...
    with c_right:
        st.subheader("📜 LỊCH SỬ ĐỐI SOÁT (Nháy & Chi tiết)")
        st.table(pd.DataFrame(st.session_state['history']))
        
        st.divider()
        st.subheader("🎯 LẤY QUÂN TÙY CHỈNH")
        num_pick = st.number_input("Số lượng quân (từ cao xuống thấp):", 1, 100, 10)
        st.code(", ".join(df_display.head(num_pick)['Số'].tolist()))
        st.download_button("💾 XUẤT FILE JSON", data=json.dumps(st.session_state['db']), file_name="matrix_data.json")
